...
type BreastCancerSignsArticleProps = {
...
const articleTrackingName = "breast_cancer_signs_article";
...
const articleEngagedTrackedKey = "rainbowhub.articleEngagedTracked:breast-cancer-signs";
...
export function BreastCancerSignsArticle({ rewardedAdUnitPath }: BreastCancerSignsArticleProps) {
  ...
  async function startArticle() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, "breast_cancer_signs_start");
    ...
    setStartUnlocking(false);
    window.setTimeout(() => {
      document.getElementById("breast-cancer-signs-start")?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 50);
  }
  ...
  async function unlockNextSet() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, `breast_cancer_signs_unlock_${visibleCount}`);
      if (result === "granted") break;
      if (result === "closed_without_reward") {
    ...
    <article className="timed-demo legacy-quiz timed-brand-article timed-life-article">
      <div className="legacy-main">
        <section className="legacy-card legacy-start">
          ...
          <h1>Breast Cancer Signs Women Often Ignore</h1>
          <p className="legacy-sub">Some changes are easy to explain away. These are the signs worth asking a clinician about.</p>
      ...
      {started ? (
        <section id="breast-cancer-signs-start" className="timed-gallery timed-article" aria-label="Breast cancer signs article">
          ...
          {visibleItems.map((item, index) => (
            <article className="timed-photo-card timed-article-section" id={`breast-cancer-sign-${index + 1}`} key={`${item.title}-${index}`}>
          ...
          {hasMore ? (
            ...
            <aside className="timed-finished">
              ...
              <p>That is the full list. This article is general information only and is not medical advice, diagnosis, or a screening tool. Breast cancer signs can be subtle, and these symptoms can also be caused by many non-cancer conditions. If you notice new, persistent, or worrying changes, speak with a qualified healthcare professional.</p>
            </aside>
          )}
        </section>
      ) : null}
