...
const articleTrackingName = "beach_moments_article";
...
const articleEngagedTrackedKey = "rainbowhub.articleEngagedTracked:beach";
...
export function BeachArticle({ rewardedAdUnitPath }: BeachArticleProps) {
  ...
  async function startArticle() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, "beach_start");
    ...
    setStartUnlocking(false);
    window.setTimeout(() => {
      document.getElementById("beach-start")?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 50);
  }
  ...
  async function unlockNextSet() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, `beach_unlock_${visibleCount}`);
      if (result === "granted") break;
      if (result === "closed_without_reward") {
  ...
  return (
    <article className="timed-demo legacy-quiz timed-brand-article timed-beach-article">
      <div className="legacy-main">
        <section className="legacy-card legacy-start">
          <div className="legacy-badge" aria-hidden="true">
            <span>🏖️</span>
          </div>
          <h1>Beach Photos That Were Lucky To Be Caught On Camera</h1>
          <p className="legacy-sub">These beach photos start normal, then something in the background changes everything.</p>
          ...
          <button className="legacy-primary" type="button" onClick={startArticle} disabled={startUnlocking}>
            {startUnlocking ? "Loading Ad.." : <><span aria-hidden="true">▶</span> See Photos</>}
          </button>
      ...
      {started ? (
        <section id="beach-start" className="timed-gallery timed-article" aria-label="Funny beach moments article">
          <header className="timed-gallery__intro timed-article-header timed-life-intro">
            <h2>Beach Photos That Were Lucky To Be Caught On Camera</h2>
            <p className="timed-article-byline">By The Rainbow Hub</p>
            <p className="timed-life-summary">{articleData.summary}</p>
          </header>

          {visibleItems.map((item, index) => (
            <article className="timed-photo-card timed-article-section" id={`beach-moment-${index + 1}`} key={`${item.title}-${index}`}>
          ...
          {hasMore ? (
            <aside className="timed-unlock" aria-live="polite">
              ...
              <h2>Unlock the next {nextCount} moments</h2>
            ...
            <aside className="timed-finished">
              ...
              <p>That is the full list. These rewritten beach moments are made for light entertainment, using the permitted source images with fresh captions for this version.</p>
            </aside>
          )}
        </section>
      ) : null}
