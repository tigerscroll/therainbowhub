...
type CatWarningSignsArticleProps = {
...
const articleTrackingName = "cat_article";
...
const articleEngagedTrackedKey = "rainbowhub.articleEngagedTracked:cat";
...
export function CatWarningSignsArticle({ rewardedAdUnitPath }: CatWarningSignsArticleProps) {
  ...
  async function startArticle() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, "cat_start");
    ...
    setStartUnlocking(false);
    window.setTimeout(() => {
      document.getElementById("cat-start")?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 50);
  }
  ...
  async function unlockNextSet() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, `cat_unlock_${visibleCount}`);
      if (result === "granted") break;
      if (result === "closed_without_reward") {
  ...
  return (
    <article className="timed-demo legacy-quiz timed-brand-article timed-historical-article">
      ...
      {started ? (
        <section id="cat-start" className="timed-gallery timed-article" aria-label="Cat warning signs article">
          ...
          {visibleItems.map((item, index) => (
            <article className="timed-photo-card timed-article-section" id={`cat-warning-sign-${index + 1}`} key={`${item.title}-${index}`}>
          ...
          {hasMore ? (
            ...
            <aside className="timed-finished">
              ...
              <p>That is the full list. These cat-behavior notes are general reading only, not veterinary advice. If your cat has sudden behavior changes, pain, appetite changes, or seems unwell, speak with a qualified vet.</p>
            </aside>
          )}
        </section>
      ) : null}
