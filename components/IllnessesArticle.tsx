...
const articleDisclaimer = articleData.disclaimer as string | undefined;
const articleTrackingName = "illnesses_article";
...
const articleEngagedTrackedKey = "rainbowhub.articleEngagedTracked:illnesses";
...
export function IllnessesArticle({ rewardedAdUnitPath }: IllnessesArticleProps) {
  ...
  async function startArticle() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, "illnesses_start");
    ...
    setStartUnlocking(false);
    window.setTimeout(() => {
      document.getElementById("illnesses-start")?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 50);
  }
  ...
  async function unlockNextSet() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, `illnesses_unlock_${visibleCount}`);
      if (result === "granted") break;
      if (result === "closed_without_reward") {
    ...
    <article className="timed-demo legacy-quiz timed-brand-article timed-life-article">
      <div className="legacy-main">
        <section className="legacy-card legacy-start">
          ...
          <h1>Illnesses You May Inherit From Your Mother</h1>
          <p className="legacy-sub">Your mother's side of the family can reveal health clues most people never think to check.</p>
          ...
          <button className="legacy-primary" type="button" onClick={startArticle} disabled={startUnlocking}>
            {startUnlocking ? "Loading Ad.." : <><span aria-hidden="true">▶</span> See Illnesses</>}
          </button>
      ...
      {started ? (
        <section id="illnesses-start" className="timed-gallery timed-article" aria-label="Inherited health conditions article">
          ...
          {visibleItems.map((item, index) => (
            <article className="timed-photo-card timed-article-section" id={`illness-${index + 1}`} key={`${item.title}-${index}`}>
          ...
          {hasMore ? (
            <aside className="timed-unlock" aria-live="polite">
              ...
              <h2>Unlock the next {nextCount} conditions</h2>
            ...
            <aside className="timed-finished">
              ...
              <p>{articleDisclaimer ?? "This article is general information only and is not medical advice. Speak with a qualified healthcare professional or genetic counselor about personal risk."}</p>
            </aside>
          )}
        </section>
      ) : null}
