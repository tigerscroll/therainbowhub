...
const articleTrackingName = "beach_gallery_article";
...
const articleEngagedTrackedKey = "rainbowhub.articleEngagedTracked:beach2";
...
export function BeachGalleryArticle({ rewardedAdUnitPath }: BeachGalleryArticleProps) {
  ...
  async function goToNextSlide() {
    ...
    if (!shouldRequireRewarded) {
      ...
      window.setTimeout(() => {
        document.getElementById("beach2-current-slide")?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 50);
      return;
    }
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, `beach2_slide_${nextSlideNumber}`);
  ...
  return (
    <article className="timed-demo legacy-quiz timed-brand-article timed-beach-article timed-beach2-article">
      <section id="beach2-start" className="timed-gallery timed-article" aria-label="Beach moments gallery article">
        ...
        {hasMore ? (
          <div className="timed-gluten-nav timed-beach2-nav" aria-live="polite">
          ...
          <aside className="timed-finished timed-gluten-finished timed-beach2-finished">
