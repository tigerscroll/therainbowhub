...
type ProstateCancerWarningSignsArticleProps = {
...
const articleTrackingName = "prostate_cancer_warning_signs_article";
...
const articleEngagedTrackedKey = "rainbowhub.articleEngagedTracked:prostate";
...
export function ProstateCancerWarningSignsArticle({ rewardedAdUnitPath }: ProstateCancerWarningSignsArticleProps) {
  ...
  async function startArticle() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, "prostate_warning_signs_start");
    ...
    setStartUnlocking(false);
    window.setTimeout(() => {
      document.getElementById("prostate-start")?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 50);
  }
  ...
  async function unlockNextSet() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, `prostate_warning_signs_unlock_${visibleCount}`);
      if (result === "granted") break;
      if (result === "closed_without_reward") {
    ...
    <article className="timed-demo legacy-quiz timed-brand-article timed-life-article">
      <div className="legacy-main">
        <section className="legacy-card legacy-start">
          ...
          <h1>Prostate Cancer Signs Men Often Ignore</h1>
          <p className="legacy-sub">Some changes are easy to dismiss. These are the signs worth asking a doctor about.</p>
          ...
          <button className="legacy-primary" type="button" onClick={startArticle} disabled={startUnlocking}>
            {startUnlocking ? "Loading Ad.." : <><span aria-hidden="true">▶</span> See Signs</>}
          </button>
      ...
      {started ? (
        <section id="prostate-start" className="timed-gallery timed-article" aria-label="Prostate cancer warning signs article">
          <header className="timed-gallery__intro timed-article-header timed-life-intro">
            <h2>Prostate Cancer Warning Signs</h2>
            <p className="timed-article-byline">By The Rainbow Hub</p>
            <p className="timed-life-summary">{articleData.summary}</p>
          </header>

          {visibleItems.map((item, index) => (
            <article className="timed-photo-card timed-article-section" id={`prostate-warning-sign-${index + 1}`} key={`${item.title}-${index}`}>
          ...
          {hasMore ? (
            ...
            <aside className="timed-finished">
              ...
              <p>That is the full list. This article is general information only and is not medical advice, diagnosis, or a screening tool. Prostate cancer can have no early symptoms, and these signs can be caused by many non-cancer conditions. If you notice new, persistent, or worrying changes, speak with a qualified healthcare professional.</p>
            </aside>
          )}
        </section>
      ) : null}
