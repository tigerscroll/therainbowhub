...
type HospiceNursesArticleProps = {
...
const articleTrackingName = "hospice_nurses_article";
...
const articleEngagedTrackedKey = "rainbowhub.articleEngagedTracked:hospice-nurses";
...
export function HospiceNursesArticle({ rewardedAdUnitPath }: HospiceNursesArticleProps) {
  ...
  async function startArticle() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, "hospice_nurses_start");
    ...
    setStartUnlocking(false);
    window.setTimeout(() => {
      document.getElementById("hospice-nurses-start")?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 50);
  }
  ...
  async function unlockNextSet() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, `hospice_nurses_unlock_${visibleCount}`);
      if (result === "granted") break;
      if (result === "closed_without_reward") {
  ...
  return (
    <article className="timed-demo legacy-quiz timed-brand-article timed-life-article">
      ...
      {started ? (
        <section id="hospice-nurses-start" className="timed-gallery timed-article" aria-label="Hospice nurses article">
          ...
          {visibleItems.map((item, index) => (
            <article className="timed-photo-card timed-article-section" id={`hospice-nurse-sign-${index + 1}`} key={`${item.title}-${index}`}>
