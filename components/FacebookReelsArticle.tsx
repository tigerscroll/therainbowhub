...
type FacebookReelsArticleProps = {
...
const articleStages = articleData.stages as ArticleStage[];
const articleTrackingName = "facebook_reels_money_article";
...
const articleEngagedTrackedKey = "rainbowhub.articleEngagedTracked:facebook-reels";
...
export function FacebookReelsArticle({ rewardedAdUnitPath }: FacebookReelsArticleProps) {
  ...
  const visibleStages = useMemo(() => articleStages.slice(0, visibleCount), [visibleCount]);
  const hasMore = visibleCount < articleStages.length;
  ...
  async function startArticle() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, "facebook_reels_start");
    ...
    setStartUnlocking(false);
    window.setTimeout(() => {
      document.getElementById("facebook-reels-start")?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 50);
  }
  ...
  async function unlockNextStage() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      result = await requestRewardedAd(rewardedAdUnitPath, `facebook_reels_unlock_${visibleCount + 1}`);
      if (result === "granted") break;
      if (result === "closed_without_reward") {
        setStatus("The ad was closed early. Try again to unlock the next stage.");
        setUnlocking(false);
        return;
      }
    ...
    setVisibleCount((count) => Math.min(count + 1, articleStages.length));
    setStatus(result === "granted" ? "Unlocked." : "");
    setUnlocking(false);
  }

  return (
    <article className="timed-demo legacy-quiz timed-brand-article timed-reels-article">
        ...
        <section className="legacy-card legacy-start">
          ...
          <div className="timed-meta-row" aria-label="Article details">
            <span>
              ...
              <strong>8 minute<br />read</strong>
            </span>
            <span>
          ...
          <button className="legacy-primary" type="button" onClick={startArticle} disabled={startUnlocking}>
            {startUnlocking ? "Loading Ad.." : <><span aria-hidden="true">▶</span> {articleData.ctaLabel}</>}
          </button>
      ...
      {started ? (
        <section id="facebook-reels-start" className="timed-gallery timed-article" aria-label="Facebook Reels money article">
          ...
          {visibleStages.map((stage, index) => (
            <article className="timed-photo-card timed-article-section timed-reels-stage" id={`facebook-reels-stage-${index + 1}`} key={stage.title}>
              <header className="timed-section-heading">
                <span className="timed-section-number" aria-hidden="true">{index + 1}</span>
                <div>
                  <p className="timed-kicker">{stage.eyebrow}</p>
                  <h3>{stage.title}</h3>
                </div>
              </header>
              ...
              <div className="timed-reels-takeaway">
                <strong>Quick checklist</strong>
                <ul>
                  {stage.takeaways.map((takeaway) => (
          ...
          {hasMore ? (
            <aside className="timed-unlock" aria-live="polite">
              <p className="timed-kicker">Next stage</p>
              <h2>Unlock stage {visibleCount + 1}</h2>
              <p>View a short ad to continue.</p>
              <button className="legacy-primary" type="button" onClick={unlockNextStage} disabled={unlocking}>
                {unlocking ? "Loading Ad.." : "Continue Reading"}
              </button>
              <span className="timed-ad-note">{unlocking ? <>When the ad ends, <b>tap the X</b> in the top right to continue.</> : "Short ad first — then the next stage unlocks."}</span>
