...
for (const folder of folders) {
  ...
  if (folder.name === "memory") {
    ...
    fail(source.landing?.startPrompt?.eyebrow === "READY TO START" && source.landing?.startPrompt?.icon === "✓" && source.landing?.startPrompt?.title === "Memory test ready" && source.landing?.startPrompt?.copy === "Watch a short ad to begin." && source.landing?.startPrompt?.button === "Continue →", `${folder.name}/en.json: Memory rewarded Start prompt changed.`);
