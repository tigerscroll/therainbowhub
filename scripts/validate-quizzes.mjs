...
for (const folder of folders) {
  ...
  if (folder.name === "memory") {
    ...
    fail(config.engine?.targetRatio === 0.8 && config.engine?.rewarded?.start === true && config.engine?.rewarded?.stages === true && config.engine?.rewarded?.attempts === 3 && config.engine?.rewarded?.confirmStart === true, `${folder.name}: Memory target and rewarded flow changed.`);
    ...
    fail(source.landing?.startNote === undefined, `${folder.name}/en.json: Memory landing must keep rewarded-ad copy inside the confirmation prompt only.`);
    fail(source.landing?.startPrompt?.eyebrow === "READY TO START" && source.landing?.startPrompt?.icon === "✓" && source.landing?.startPrompt?.title === "Your memory test is ready" && source.landing?.startPrompt?.copy === "Watch one short ad, then the test begins." && source.landing?.startPrompt?.button === "Continue →", `${folder.name}/en.json: Memory rewarded Start prompt changed.`);
