...
for (const slug of args.length ? args : slugs) {
  ...
  Object.values(copy.results.profiles).forEach((profile, i) => { profile.copy = descriptions[i]; });
  if (slug === 'chef') {
  ...
  for (const [roundIndex, round] of content.rounds.entries()) {
    ...
    copy.career.stages[stageId] = {
      ...
      preAdCopy: final ? 'Did you reach 80%? Your score and strengths are ready.' : round.feedback,
      preAdButton: final ? 'See My Result' : 'Continue',
      ...(final ? { preAdChecks: ['Answers checked', 'Topic strengths compared', 'Score calculated'] }
    ...
    for (const [questionIndex, row] of round.questions.entries()) {
      ...
      assert.ok(categories.includes(category), `${id}: unknown category ${category}`);
      ...
      manifest.structure.questions[id] = { presentation: 'text', correctAnswerId: `a${correctIndex + 1}`, category, answerIds: ['a1', 'a2', 'a3', 'a4'] };
      ...
      key.push({ id, correct, question, rationale, sourceUrl });
    }
  }
