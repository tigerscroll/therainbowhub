...
try {
  for (const width of (process.env.QUIZ_TEST_WIDTHS ?? "320,390,1440").split(",").map(Number)) {
    ...
    await page.screenshot({ path: `/tmp/${slug}-polish-landing-${width}.png`, fullPage: true, animations: "disabled" });
    ...
    for (const [stageIndex, stage] of manifest.structure.stages.entries()) {
      for (const [index, id] of stage.questionIds.entries()) {
        ...
        if (manifest.structure.questions[id].study) {
          const board = question.locator(".quiz-engine__study");
          await board.waitFor();
      ...
      if (stageIndex === 0) {
        ...
        await page.screenshot({ path: `/tmp/${slug}-polish-checkpoint-${width}.png`, fullPage: true });
      }
      assert.match(await checkpoint.locator(".quiz-engine__checkpoint-ad-note").innerText(), /short ad/i);
      if (stageIndex === 0 && width === 390) {
        await page.screenshot({ path: `/tmp/${slug}-rewarded-checkpoint.png`, fullPage: true });
        await page.reload();
        await checkpoint.waitFor();
