...
async function journey(score,width){
 ...
 try{
  ...
  if(score===8)await page.screenshot({path:'/tmp/mechanic-en-landing.png'});
  ...
  for(const [s,stage]of m.structure.stages.entries()){
   for(const id of stage.questionIds){
    ...
    if(score===8&&id==='mechanic-r5q1')await page.screenshot({path:'/tmp/mechanic-en-scenario.png'});
   ...
   if(score===8&&s===0)await page.screenshot({path:'/tmp/mechanic-en-checkpoint.png'});
  ...
  if(score===8)await page.screenshot({path:'/tmp/mechanic-en-result.png'});
