...
const m=JSON.parse(fs.readFileSync('data/quizzes/mechanic/quiz.json'));
const e=JSON.parse(fs.readFileSync('data/quizzes/mechanic/en.json'));
...
async function journey(score,width){
 ...
 try{
  ...
  if(score===24)await page.screenshot({path:'/tmp/mechanic-en-landing.png'});
  ...
  for(const [s,stage]of m.structure.stages.entries()){
   for(const id of stage.questionIds){
    ...
    if(score===24&&s===4&&id.endsWith('q1'))await page.screenshot({path:'/tmp/mechanic-en-scenario.png'});
   ...
   if(score===24&&s===0)await page.screenshot({path:'/tmp/mechanic-en-checkpoint.png'});
  ...
  if(score===24)await page.screenshot({path:'/tmp/mechanic-en-result.png'});
